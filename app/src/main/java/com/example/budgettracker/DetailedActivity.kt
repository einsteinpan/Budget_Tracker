...
class DetailedActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateBtn.setOnClickListener {
            ...
            if (label.isEmpty()) {
                labelLayout.error = "請選擇類別"
            } else if (amount == null) {
                amountLayout.error = "請輸入正確金額"
            } else {
                // 使用原有 transaction 物件的日期
                val updatedTransaction = Transaction(transaction.id, label, amount, description, transaction.date)
